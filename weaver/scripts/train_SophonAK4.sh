...
ARG="--network-config networks/pheno2/example_SophonAK4.py -o num_classes 23 -o embed_dims [64,256,64] -o pair_embed_dims [32,32,32] \
--use-amp --batch-size 512 --start-lr 5e-4 --samples-per-epoch $((4000 * 1024 / $NGPUS)) --samples-per-epoch-val $((1000 * 1024 / $NGPUS)) --num-epochs 80 --optimizer ranger \
--num-workers 5 --fetch-step 0.01 \
--data-train ${DATADIR}/train_higgs2p_ak4_ntuple/ntuples_*.root \
--data-config $config \
