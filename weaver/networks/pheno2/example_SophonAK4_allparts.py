...
def evaluate_classification_sophon(model, test_loader, dev, epoch, for_training=True, loss_func=None, steps_per_epoch=None,
    ...
    if tb_helper:
        scores_dict = {
            'HH4b': np.sum(scores[:, :136], axis=1),
            'QCD': scores[:, 136],
            'ttbar': scores[:, 137]
            # 'SingleBoson': scores[:, 138]
        }
        flag_dict = {
            'HH4b': (labels['truth_label'] < 136) & (labels['truth_label'] >= 0) ,  
            'QCD': (labels['truth_label'] == 136),
            'ttbar': (labels['truth_label'] == 137)
            # 'SingleBoson': (labels['truth_label'] == 138)
        }
        comp_list = [('HH4b', 'QCD'), ('HH4b', 'ttbar')] # ROC curves for A vs B
        bkgrej = {}
        ...
        ax.set_xlim(1e-3, 1); ax.set_ylim(1e-7, 1), ax.set_yscale('log'), ax.set_xscale('log')
